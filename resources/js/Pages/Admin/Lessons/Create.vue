...
<template>
    ...
    <AdminLayout>
        <template #title>レッスン作成</template>
        ...
        <div class="max-w-2xl">
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div class="rounded-xl bg-slate-800 border border-slate-700 p-6 space-y-6">
                    ...
                    <!-- Content -->
                    <div v-if="form.type !== 'video'">
                        <label class="block text-sm font-medium text-slate-300 mb-2">コンテンツ（HTML可）</label>
                        <textarea
                            v-model="form.content"
                            rows="8"
                            class="w-full rounded-lg bg-slate-900 border-slate-600 text-white placeholder-slate-500 focus:border-cyan-500 focus:ring-cyan-500 font-mono text-sm"
                            placeholder="<h3>セクション1</h3><p>内容を入力...</p>"
                        ></textarea>
                    </div>
